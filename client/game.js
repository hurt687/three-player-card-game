...
function cardName(card) { return card.joker ? card.rank : `${card.suit}${card.rank}`; }
function renderHand() {
...
document.querySelector("#playButton").onclick = () => { if (selectedIndex === null) { status.textContent = "请先选择一张牌"; return; } socket.send(JSON.stringify({ type: "play", index: selectedIndex, asRank: document.querySelector("#jokerRank").value })); };
